...
typedef struct pattern {
  ...
  double **features; // fbank feature vectors
  int    length;
} PATTERN;
...
typedef struct struct_learn_parm {
  ...
  int   num_features;         /* number of features in each
          individual token vector */
  int   num_classes;          /* number of classes that each feature
          can take, i.e., number of types of labels */
} STRUCT_LEARN_PARM;
